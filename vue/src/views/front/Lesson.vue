          ...
          <el-carousel height="300px">
            <div class="overlay">
              <div class="overlay-content">
                <div>
                  <h2 class="font-title" style="text-align: left">北邮南太孔院教务系统    {{ className }}</h2>
            ...
            <el-carousel-item v-for="item in carouselData" :key="item">
              <img :src="item" class="lesson-carousel-image">
            </el-carousel-item>
          </el-carousel>
        </div>
      </div>
        ...
        <div class="table" >
          <el-table class="custom-table" :data="filteredTableData" stripe>
            ...
            <el-table-column  class="custom-table__expanded-cell" prop="name" show-overflow-tooltip width="600">
              <template v-slot="scope">
                <a :href="'/front/LessonDetail?id=' + scope.row.id" style="color: rgba(51,51,51,0.78);font-size: 15px">{{ scope.row.name }}</a>
              </template>
            </el-table-column>
...
export default {
  ...
  mounted() {
    this.load(1)
    this.getClassName(); // 获取班级名称
  },
  // methods：本页面所有的点击事件或者其他函数定义区
  methods: {
    load(pageNum) {
      if (pageNum) this.pageNum = pageNum;
      const classId = this.user.classId; // 获取用户的 classId
      this.$request.get('/lesson/selectPageByClassId', {
        params: {
          classId: classId,
    ...
    filterData() {
      if (this.value) { // 如果选择了内容类型
        this.$request.get('/lesson/selectByType', {
          params: {
            type: this.value,
            pageNum: this.pageNum,
            pageSize: this.pageSize,
    ...
    getClassName() {
      ...
      if (classId) {
        this.$request.get(`/classes/selectById/${classId}`).then(res => {
          if (res.code === '200') {
            this.className = res.data?.name || '未知班级';
          } else {
